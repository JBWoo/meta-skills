...
2. Read both screenshots using the available image-viewing capability in a single evaluation step (for local files, prefer `view_image` when available)
...
- Windows: `Start-Process dashboard_url`
...
3. **Make the change.** Edit the target file(s) at the chosen mutation level.
   - Before editing, checkpoint only the files you plan to touch in this experiment (`SKILL.md`, linked `references/`, and `agents/openai.yaml` if relevant).
...
6. **Decide: keep or discard.**
   ...
   - Score unchanged -> **DISCARD.** Restore only the checkpointed files from this experiment.
   - Score worse -> **DISCARD.** Restore only the checkpointed files from this experiment.
...
- Never use broad repo resets or revert unrelated user changes.
...
- Prefer experiment-local checkpoints over VCS-wide rollback commands.
