...
name: reflect
description: >-
  Session wrap-up and fix-learning capture skill for Codex. Use when the user
  explicitly asks for "$reflect", "reflect", "session reflect", "end session",
  "세션 정리", "오늘 한 거 정리", or "마무리", or when the user clearly confirms
  that a concrete issue from the current coding session is fixed ("고쳐졌다",
  ...
  Reflect.
metadata:
  short-description: Wrap up a session or capture a confirmed fix
---
...
- Default architecture: single agent, inline analysis, no subagents
...
- Review the last 5-10 turns plus relevant `git status --short` / diff context
...
If the eligibility gate fails during implicit invocation, behave as though the skill did not apply:
...
- files changed in this session from `git status --short` and relevant diff
