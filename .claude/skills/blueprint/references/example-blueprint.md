...
**트레이드오프**: 신중함 > 속도. n8n 배포 실패는 재작업 비용이 크므로 Step 5 검증 루프에서 충분히 확인한다. 단순한 설정 변경에는 판단력을 사용해 과도한 검증을 피한다.

**이 가이드라인이 잘 작동하고 있다면:**
- Step 4 사용자 컨펌 전에 가정과 선택지가 명시적으로 제시된다
- Execute Command 등 금지 패턴을 시도하지 않고 대안으로 직행한다
- 검증 에러 발생 시 추측성 수정 대신 validation-expert 스킬을 먼저 호출한다
